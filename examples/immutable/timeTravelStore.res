...
let goBack = currentState =>
  ...
  | Some(lastState) =>
    ...
    if Belt.List.length(past.contents) > 0 {
      past := past.contents ->Belt.List.drop(1) -> Belt.Option.getExn
    }
    lastState
  | None => currentState
  }
...
let goForward = currentState =>
  ...
  | Some(nextState) =>
    ...
    if Belt.List.length(future.contents) > 0 {
      future := future.contents ->Belt.List.drop(1) -> Belt.Option.getExn
    }
    nextState
  | None => currentState
  }
